...
export async function listCollabDocs(filter: ListCollabDocsFilter = {}): Promise<{ items: CollabDoc[]; total: number }> {
  const r = await get<ApiEnvelope<CollabDoc[]> & { total?: number }>('/collaborative-docs', { params: filter as Record<string, unknown> })
...
export async function createCollabDoc(payload: { kb_id: string; title: string; doc_kind?: CollabDocKind }): Promise<CollabDoc> {
...
export async function updateCollabDoc(id: string, payload: { title?: string; visibility?: string }): Promise<CollabDoc> {
  const r = await patch<ApiEnvelope<CollabDoc>>(`/collaborative-docs/${id}`, payload)
  return r.data
}
...
export async function listCollabDocPresence(id: string): Promise<CollabDocSession[]> {
  const r = await get<ApiEnvelope<CollabDocSession[]>>(`/collaborative-docs/${id}/presence`)
  return r.data ?? []
}
...
export async function downloadCollabDocBytes(id: string): Promise<Blob> {
  const res = await get<Blob>(`/collaborative-docs/${id}/download`, { responseType: 'blob' })
  return res
}
