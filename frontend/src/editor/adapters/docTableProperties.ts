/**
 * v0.7.54 — DOC table properties commands (copy from genoffice
 * apps/docs/src/renderer/editor/table-properties.ts, node names adapted to
 * TipTap's `table`/`tableCell`/`tableHeader` and types from the local
 * docx-engine).
 */
import type { Node as PmNode } from '@tiptap/pm/model'
import type { Command, EditorState, Transaction } from '@tiptap/pm/state'
import { isInTable, selectedRect } from '@tiptap/pm/tables'
import type { TableAutoFitMode, TableLook } from '../engines/docx-engine'
...
function selectedTable(state: EditorState): SelectedTable | null {
  ...
  try {
    ...
    return node?.type.name === 'table' ? { node, pos, tableStart: rect.tableStart, rect } : null
  } catch {
    return null
  }
}
