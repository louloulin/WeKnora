/**
 * csvImport — CSV → minimal xlsx conversion for the open-file path.
 * Vendored from genoffice apps/sheets/src/gateway/csv-import.ts (v0.7.58,
 * Tencent Docs compatibility layer: Tencent Docs / Excel export CSV in the
 * system legacy charset — GBK on Chinese Windows — so decoding must sniff
 * UTF-8 first, then the legacy charsets).
 *
 * Adapted: Buffer → Uint8Array (browser), nodebuffer → uint8array.
 */
 ...
 */
export async function sheetCsvToXlsxBuffer(csvText: string, sheetName = 'Sheet1'): Promise<Uint8Array> {
...
export async function blankXlsxBuffer(sheetName = 'Sheet1'): Promise<Uint8Array> {
...
/// CSV export of the active sheet grid: display strings, Excel-style quoting,
/// CRLF rows, UTF-8 with a BOM so Excel / Tencent Docs reopen Chinese correctly.
/// Vendored from genoffice apps/sheets/src/renderer/csv-export.ts (pure parts).
...
/** Serialize a sheet grid to CSV bytes (UTF-8 BOM + CRLF). */
export function gridToCsvBytes(rows: readonly (readonly string[])[]): Uint8Array {
  const text = '\ufeff' + csvFromDisplayRows(rows)
