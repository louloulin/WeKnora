/// WorkbookChartEdit shape (vendored from genoffice's shared/desktop-api).
/// Kept narrow: only the fields applyChartEdit consumes. New fields added to
/// the upstream schema do NOT flow through unless this list is updated.
interface WorkbookChartEditSeries {
...
interface WorkbookChartEditSeriesEntry extends WorkbookChartEditSeries {
  readonly index: number
}

interface WorkbookChartEditAxisTitles {
  readonly category?: string | null
  readonly value?: string | null
}

interface WorkbookChartEditValueAxis {
...
export interface WorkbookChartEdit {
  ...
  readonly chartType?: 'column' | 'bar' | 'line' | 'area' | 'pie' | 'doughnut'
  readonly seriesColors?: Readonly<Record<string, string>>
  readonly legend?: 'none' | 'right' | 'bottom' | 'top' | 'left'
  readonly dataLabels?: 'none' | 'value' | 'percent' | 'category-percent'
  readonly dataLabelPosition?: 'center' | 'inside-end' | 'outside-end'
  readonly dataLabelFormat?: string
  readonly axisTitles?: WorkbookChartEditAxisTitles
  readonly pointColors?: Readonly<Record<string, Readonly<Record<string, string>>>>
  readonly grouping?: 'clustered' | 'stacked' | 'percentStacked'
  readonly gridlines?: boolean
  readonly valueAxis?: WorkbookChartEditValueAxis
  readonly gapWidthPct?: number
  readonly holeSizePct?: number
  readonly explosionPct?: number
  readonly pointExplosions?: Readonly<Record<string, number>>
  readonly seriesSet?: readonly WorkbookChartEditSeries[]
  readonly series?: readonly WorkbookChartEditSeriesEntry[]
}
