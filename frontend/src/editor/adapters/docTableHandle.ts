/**
 * v0.7.55 — DOC table move handle (copy from genoffice
 * apps/docs/src/renderer/editor/table-handle.ts, node name adapted to
 * TipTap's `table` and the i18n tooltip inlined).
 *
...
const TABLE_HANDLE_TIP = '移动表格'

/** absolute pos of the table ancestor under the given viewport coords (null = not in a table) */
function tablePosAt(view: EditorView, coords: { left: number; top: number }): number | null {
  ...
  if (found.inside >= 0 && view.state.doc.nodeAt(found.inside)?.type.name === 'table') {
    return found.inside
  }
  return null
}
...
function tablePosFromTarget(view: EditorView, target: EventTarget | null): number | null {
  ...
  const table = element?.closest('table.collab-doc-pro__table')
  if (!(table instanceof HTMLElement) || !view.dom.contains(table)) return null
  let found: number | null = null
  view.state.doc.descendants((node, pos) => {
    if (found !== null) return false
    if (node.type.name === 'table' && view.nodeDOM(pos) === table) {
      found = pos
      return false
    }
    return true
...
const MOVE_SVG =
  ...
  '<path d="M8 1v14M1 8h14M8 1 6 3M8 1l2 2M8 15l-2-2M8 15l2 2M1 8l2-2M1 8l2 2M15 8l-2-2M15 8l2 2"/>' +
  '</svg>'

export const DocTableHandle = Extension.create({
  name: 'docTableHandle',
...
function tableHandlePlugin(): Plugin {
  return new Plugin({
    key: new PluginKey('docTableHandle'),
    view(view) {
      ...
      handle.title = TABLE_HANDLE_TIP
      handle.setAttribute('aria-label', TABLE_HANDLE_TIP)
      ...
      const selectTable = (): NodeSelection | null => {
        ...
        if (view.state.doc.nodeAt(tablePos)?.type.name !== 'table') return null
        try {
