/**
 * docCompare — DOC paragraph-level diff (v0.7.69).
 *
 * Vendored verbatim from genoffice apps/docs/src/renderer/editor/compare.ts
 * (Word "Review > Compare" panel). Pure functions only — no Vue, no DOM, no
 * Editor instance — so tests run in plain node.
 *
 * Adapted: Block import path changed from '@genoffice/docx-engine' to our
 * local engine.
 */
import type { Block } from '../engines/docx-engine/index'
