/// v0.7.43.c — vendored from genoffice xlsx-styles.ts with desktop-api
/// zod schema inlined (we don't need zod for this adapter). short-date
/// locale handling dropped (Excel renders builtin 14/22 with the OS
/// regional setting, but the WeKnora editor writes explicit number-format
/// strings instead).
...
function toArgb(hexColor: string): string {
  // Accept either 6-char RGB (#RRGGBB without '#') or 8-char ARGB (#AARRGGBB).
  // OOXML requires 8-char ARGB in rgb="..." attributes, so pad alpha to FF when
  // the caller passes the 6-char form. Previously this assumed a 7-char hex
  // and would chop the second character of an 8-char ARGB, producing a
  // 9-char malformed string (e.g. 'FF0000FF' → 'FFF0000FF').
