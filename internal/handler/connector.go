...
func (h *ConnectorHandler) resolveCtx(c *gin.Context) (tenantID, userID string, ok bool) {
	...
	uv, _ := c.Get("user_id")
	tid, ok1 := anyToString(tv)
	uid, _ := anyToString(uv)
	if !ok1 {
...
// anyToString converts a gin context value to its underlying string.
// Returns (value, true) when the value is a string; ("", false)
// otherwise. We deliberately avoid strconv.FormatUint for tenant IDs
// because the handler contract keeps the raw string form (matches the
// JWT claims shape).
