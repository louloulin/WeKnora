...
func BuildContainer(container *dig.Container) *dig.Container {
	...
	// Verified Knowledge Engine (Build #48) — manual review state on
	// top of the Build #29 AI verification scanner. Same repo handle
	// as the wiki page service so writes hit the same row.
	must(container.Provide(service.NewWikiVerificationService))
	must(container.Provide(handler.NewWikiVerificationHandler))
...
// slideActionToGovernance maps the slide service's internal action names
// (e.g. "create", "update", "auto-generate") onto the cross-product
// AuditAction constants emitted into the audit_logs table. Unknown
// actions return an empty string so the caller silently skips — the
// slide-internal audit_log table still receives the event.
func slideActionToGovernance(action string) types.AuditAction {
	switch action {
	case "create":
		return types.AuditActionSlideDeckCreated
	case "update":
		return types.AuditActionSlideDeckUpdated
	case "delete":
		return types.AuditActionSlideDeckDeleted
	case "auto-generate":
		return types.AuditActionSlideDeckAutoGenerated
	case "export":
		return types.AuditActionSlideDeckExported
	default:
		return ""
	}
}

// detailJSON wraps a free-form detail string from the slide service into
// a JSON object suitable for the audit_logs.Details JSONB column. The
// returned string is always valid JSON; on marshal error we emit an
// empty object so the row still writes.
func detailJSON(detail string) string {
	if detail == "" {
		return "{}"
	}
