...
type NodeOps[N any] interface {
	...
	// the original Cursor.toSyncCursor / FeishuDriveCursor.toSyncCursor.
		...
		for i, node := range nodes {
			...
			if ferr != nil {
				...
				if eerr := h.Emit(ctx, types.FetchedItem{
					...
					Metadata:         ErrorItemMeta(ferr, nil),
				}); eerr != nil {
					return nil, eerr
				}
			} else {
			...
			processed++
			if processed%TencentDocsStreamCheckpointInterval == 0 || time.Since(lastCheckpoint) >= TencentDocsStreamCheckpointMaxInterval {
